...
static bool hodl_got_new_work = false;

// called with work unlocked, ok to wait
void hodl_wait_for_diff( void* arg_ignored, struct work* g_work )
{
  // first thread to get here does the work, then waits for the others
  // no mutex needed, all threads will still use the same nonce
  if ( !hodl_got_new_work )
  {
     hodl_got_new_work = true;
     g_work->data[ algo_gate.nonce_index ] = 
                        swab32( ( clock() + rand() ) % 9999 );
  }
  pthread_barrier_wait( &hodl_barrier );
  hodl_got_new_work = false;
}
 
// called with work locked, can't wait
...
bool register_hodl_algo( algo_gate_t* gate )
{
  ...
  gate->wait_for_diff         = (void*)&hodl_wait_for_diff;
